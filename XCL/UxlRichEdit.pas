...
var s_text: widestring;
	i: integer;
begin
   s_text := Text;    // �����wordwrap���µ�recreate, ��ʱ�ؼ��п����������֣�ִ��settextmode����ʧ��
   Text := '';
	inherited ReCreate ();
...
begin
	SetSel (0, 0);
   Perform ( EM_GETLINECOUNT, 0, 0 );  // �޴˾䣬���������ֺ�ָ�����λ�û�����⣬��������Ϊ����δ������ȫϵͳ�ͳ��Թ�����Ե��
...
begin
   ...
   begin
      ...
      begin
         ...
         else if (i_pos > 1) and (i_prefixlen = 1) and (Ord(s_prefix[1]) <= 32) and (Ord(s[i_pos - 1]) <= 32) and (Ord(s[i_pos - 1]) <> 13) then  // shift_tab ʱ���ո��һ���س�
         begin
         	dec (i_pos);
            Delete (s, i_pos, 1);
            dec (i_len, 1);
   ...
   case AMessage of
	   WM_KEYDOWN:
         begin
            ...
            begin
               ...
               i := 1;
               n := length(s);
               while (i <= n) and ((s[i] = #9) or (s[i] = #32)) or (s[i] = '��') do   // �������Ŀո�
               begin
      ...
      WM_CONTEXTMENU:
         ...
         begin
            ...
            if SystemTimeToInt64 (Now, ittMilliSecond) - i_time <= 100 then  // ��δ����ϵͳ������Ĭ�ϲ˵������scrollbar
               FOnContextMenu (self);
            b_processed := true;
         end;
      WM_MBUTTONDOWN:
         if (wparam and MK_CONTROL) <> 0 then   // ctrl+�м����������� zoom
         begin
            Post (EM_SETZOOM, 0, 0);
            b_processed := true;
         end;
...
//      procedure SetSelColor (i_color: TColor);   // ��Ҫ��PlainText���ȥ��������Ч������ʵ���﷨������ʵ��OnPaint�¼������¼����������SetSelColor���ɡ�
