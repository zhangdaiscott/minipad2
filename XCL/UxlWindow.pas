...
begin
   ...
   begin
      ...
      lpszMenuName := nil;
      lpszClassName := PWideChar(ProgClass);
   end;

   if RegisterClassW (WindowClass) = 0 then raise Exception.create ('Window Class Register Failed!');
   ...
   b_processed := true;
      ...
      WM_Destroy :
         PostQuitMessage(0);
      	...
      	if AMessage = WM_TaskBarRestart then    // else ��Ҫ constant value�������� var
         begin
            if assigned (FTrayIcon) then
               TxlTrayIcon(FTrayIcon).DrawIcon (true);
         end
...
begin
   ...
   if GetClassNameW (hwnd, pwidechar(s_buffer), 2000) = 0 then exit;   //��õ�ǰ�������ڵ�����
   if not IsSameStr (pwidechar(s_buffer), ProgClass) then exit;

   WinInstance := GetWindowLong (hwnd, GWL_HINSTANCE);  //��õ�ǰ�������ڵ�ʵ��
   ...
   FindHwnd := hwnd;	//ʹ��ȫ�ֱ��� FindHwnd �����ҵ��ľ��
   result := false;	//�ҵ��Ժ�ͽ�������
end;
...
begin
   mymutex := createmutexW (nil, false, pwidechar(ReplaceStr (progClass, '\', '/')));
	...
	if (not result) and b_activate then    //ͬһexeֻ��������һ��ʵ�����ڶ�������ʱrestore��ǰ��ʵ����
  	begin
      FindHwnd := 0;
     	EnumWindows (@EnumWndProc, 0);    //�ص���������return false�����д��ڱ�����ɺ󷵻ء�
      if FindHwnd <> 0 then
      begin
