   ...
   begin
      ...
      if ListCenter.GetSubItem (id, i_index, s_tip, 200) then
		begin
         ...
         if FWndParent.StayOnTop and (FWndParent.Status <> wsMinimize) then   // ��������²��ܽ� Tooltip StayOnTop������˵�����ת����̨����� ToolTip ֻ����ʾ�ڲ˵��Ҳ��Է�����û
         begin
            pt.x := rt.Right + 5;
            pt.y := rt.Top;
         ...
         begin
            ...
            if PointInRect (pt, rt) then
               FMenuTip.ShowTip (s_tip)    // ���涨λ�ã���Tip����ʱʵʱȡλ��
            else
            begin    // ��Բ�����꣬���ü��̴��� wm_menuselect �ĳ��ϣ��� Tip ��ʾ�ڲ˵����²�
...
procedure TMenuManager.ExecuteCommand (opr: word);
   ...
   begin
      ...
      begin
         Initialize (o_currentlist, o_alllist, o_defaultlist);      // ����ָ�봫��
         if Execute () then
         begin
