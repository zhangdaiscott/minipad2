...
procedure TSelMemo.SetSel (id: integer; i_start, i_len, i_line: integer);    // ��Ҫ��ͼ��¼ FEditor.ScrollPos!!!
var s: widestring;
begin
...
//   if OptionMan.Options.BmkRule = 0 then   // �ʺ��ھ��󲿷���������Իָ� FEditor.ScrollPos �ᵼ�´��ı����޷��ָ�λ��bug!!!
...
begin
   with FEditor do       // ��Ҫ�����κ�ǣ�浽 IsEmpty ���жϣ����� CanClear���������ؽ��ʹ��ı��µ�ִ��Ч�ʣ�
   begin
      case opr of
         m_undo:
            result := CanUndo ();
...
begin
   ...
   case opr of
      ...
      m_insertimage:
         begin
            o_dlg := TxlOpenDialog.Create;
            o_dlg.Filter := 'Image Files (*.bmp;*.jpg;*.jpeg;*.png;*.gif)|*.bmp;*.jpg;*.jpeg;*.png;*.gif|All Files (*.*)|*.*';
            if o_dlg.Execute then
            begin
               s := o_dlg.Path + o_dlg.FileName;
               if PathFileExists(PWideChar(s)) then
               begin
                  if not PathFileExists(PWideChar(DataDir + 'images\')) then
                     CreateDir(DataDir + 'images\');
                  CopyFileW(PWideChar(s), PWideChar(DataDir + 'images\' + o_dlg.FileName), false);
                  FEditor.SetFocus;
                  FEditor.SelText := '[image:' + o_dlg.FileName + ']';
               end;
            end;
            o_dlg.Free;
         end;
   	...
   	b_processed := true;
		case AMessage of
         WM_FOCUSEDITOR:
            begin
               FEditor.SetFocus;
               if FEditor.CursorInWindow and KeyPressed(VK_LBUTTON) then   // ����ʡ�ԣ�����������ѡ�����֣�
               begin
...
//         	FEditor.FirstVisibleLine := FEditor.FirstVisibleLine;  // ��ֹ������󻯺��������ҳ��ײ�������
      e_GetEditorSelText:
